...
import { X, ArrowRight, Eye, EyeOff, Loader2, ArrowLeft } from 'lucide-react';
import SunburstIcon from './SunburstIcon';
import { signInWithEmail, signUpWithEmail, signInWithOAuth } from '../lib/supabaseClient';
...
export function SignInModal({ isOpen, onClose }) {
  ...
  const [isExistingCompleted, setIsExistingCompleted] = useState(false);
  ...
  useEffect(() => {
    ...
    async function checkCurrent() {
      try {
        ...
        if (u) {
          setExistingUser(u);
          const done = await hasCompletedOnboarding(u.id, u);
          setIsExistingCompleted(done);
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isSignUp) {
        ...
        setAuthSuccess('Account created! Redirecting to setup...');
        setTimeout(() => {
          onClose();
          navigate('/onboarding');
        }, 900);
  ...
  return (
    <div className="fixed inset-0 z-50 w-full h-full min-h-screen overflow-y-auto bg-black flex flex-col">
      ...
      <div className="w-full min-h-screen grid grid-cols-1 lg:grid-cols-2">
        ...
        <div className="relative w-full min-h-[380px] lg:min-h-screen bg-[#000000] overflow-hidden flex flex-col justify-between p-8 sm:p-14 lg:p-20 text-white">
          {/* Exact Hero Video Texture Layer */}
          <video
            ref={videoRef}
          ...
          <div className="relative z-10">
            <button
              onClick={onClose}
              className="inline-flex items-center gap-2 text-xs font-medium text-white/70 hover:text-white transition-colors cursor-pointer"
            >
          ...
          {/* Left Main Typography matching the reference */}
        ...
        <div className="relative w-full min-h-screen bg-white flex flex-col justify-center items-center p-6 sm:p-12 lg:p-16 text-slate-900">
          ...
          <button
            onClick={onClose}
            className="absolute top-6 right-6 w-10 h-10 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-black flex items-center justify-center transition-colors cursor-pointer"
            aria-label="Close"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="w-full max-w-md mx-auto space-y-6">
            ...
            {existingUser && (
              <div className="p-3.5 rounded-2xl bg-blue-50/90 border border-blue-200 text-xs text-blue-950 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <div className="font-semibold text-blue-950 text-xs">Signed in as</div>
                  <div className="text-blue-700 text-[11px] truncate">{existingUser.email}</div>
                </div>
                <button
                  type="button"
                  onClick={() => {
                    onClose();
                    navigate(isExistingCompleted ? '/chat' : '/onboarding');
                  }}
                  className="px-4 py-1.5 bg-[#0528e0] hover:bg-[#0420b5] text-white rounded-full text-xs font-semibold transition-all shrink-0 shadow-sm cursor-pointer"
                >
                  {isExistingCompleted ? 'Go to Chat →' : 'Finish Setup →'}
                </button>
              </div>
            )}
            ...
            <div className="pt-2 text-center text-xs text-slate-500">
              <span>{isSignUp ? 'Already have an account?' : 'Dont have an account?'} </span>
              <button
                type="button"
                onClick={() => {
                ...
                className="text-blue-600 hover:text-blue-700 font-semibold cursor-pointer underline-offset-2 hover:underline ml-1"
              >
