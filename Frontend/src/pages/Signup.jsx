...
import { ArrowRight, Eye, EyeOff, Loader2, ArrowLeft, AlertCircle, CheckCircle2 } from 'lucide-react';
import SunburstIcon from '../components/SunburstIcon';
import { signUpWithEmail, signInWithOAuth } from '../lib/supabaseClient';
...
export function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data, error, isDemo } = await signUpWithEmail(email, password);
      if (error) throw error;

      if (isDemo) {
        setAuthSuccess('Account created! Redirecting to onboarding...');
        ...
        setAuthSuccess('Account created! If email confirmation is enabled, check your inbox or proceed to log in.');
          ...
          <div className="w-full max-w-md mx-auto space-y-6">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="font-instrument-sans text-xs sm:text-sm text-slate-500">
                Join Koko AI to configure your personalized model orchestration.
              </p>
            </div>
